...
export async function GET(request: NextRequest, { params }: { params: { websiteId: string } }) {
  try {
    ...
    const backendUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/analytics/traffic/${params.websiteId}?period=${period}`;
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch traffic analytics' }, { status: response.status });
    }
